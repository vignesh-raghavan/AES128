      	...
      	for (i = 0; i <16; i++)
			{
				...
				//if(round < 4) printf("sss 0x%02x\n", state[i] ^ key[i]);
				//state[i]=sbox[state[i] ^ key[i]];
				state[i]=state[i] ^ key[i];
				//if(round == 0) printf("xxx 0x%02x\n", state[i]);
      	}
      	//shift rows
      	buf1 = state[1];
      	state[1] = state[5];
...
int main( void )
{
  ...
  unsigned char state1[] = {0x32, 0x67, 0x96, 0x48, 0xab, 0xc3, 0xd1, 0x59,
                              0x40, 0x5a, 0x4c, 0x8b, 0xc1, 0x37, 0x72, 0xf4};
  ...
  //ct0 = IV.
  //ct1 = aes_enc_dec(state^ct0, key1, 0);
  //ct2 = aes_enc_dec(state^ct1, key1, 0);
